...
///     设置窗口.
/// </summary>
public class SettingDefine : EditorWindow
{
    private readonly Dictionary<string, bool> dic = new Dictionary<string, bool>();
    private readonly List<MacroItem> macroItemLists = new List<MacroItem>();
    ...
    private void OnEnable()
    {
        ...
        macroItemLists.Add(
            new MacroItem {Name = "DEBUG_MODEL", DisplayName = "调试模式", isDebug = true, isRelease = false});
        ...
        for (var i = 0; i < macroItemLists.Count; i++)
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("保存", GUILayout.Width(100))) SaveMacro();
        if (GUILayout.Button("调试模式", GUILayout.Width(100)))
        {
            for (var i = 0; i < macroItemLists.Count; i++) dic[macroItemLists[i].Name] = macroItemLists[i].isDebug;
            SaveMacro();
        }

        if (GUILayout.Button("发布模式", GUILayout.Width(100)))
        {
            for (var i = 0; i < macroItemLists.Count; i++) dic[macroItemLists[i].Name] = macroItemLists[i].isRelease;
            SaveMacro();
        }
