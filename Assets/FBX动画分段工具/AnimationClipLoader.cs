    ...
    private static void 加载(string strFilePath, ModelImporter modelImporter)
    {
        ...
        try
        {
            ...
            string strConfigFileFullPath =
                strModeFileFullPath.Remove(strModeFileFullPath.LastIndexOf('.')) + $".Animation{strExtension}";
        ...
        {
            Debug.LogError($"{Selection.activeObject.name}.Animation{strExtension}文档加载失败");
            return;
        }
        ...
        for (int i = 0; i < listConfigData.Count; i++)
        {
            ...
            if (strData.Length < 3)
            {
                Debug.LogError($"{data}数据分割错误");
                continue;
            }

            if (!int.TryParse(strData[0], out int firstFrame))
            {
                Debug.LogError($"{data}第1个数据有误");
                continue;
            }

            if (!int.TryParse(strData[1], out int lastFrame))
            {
                Debug.LogError($"{data}第2个数据有误");
                continue;
            }

            if (strData[2].Equals(string.Empty))
            {
                Debug.LogError($"{data}第3个数据有误，请检查文件编码");
                continue;
            }
                ...
                using (StreamWriter reader = new StreamWriter(file))
                {
                    if (animations.Length > 0)
                    {
                        ...
                        for (int i = 0; i < animations.Length; i++)
                        {
                            ...
                            reader.WriteLine(
                                $"{animation.firstFrame:000}-{animation.lastFrame:000}-{animation.name}-{animation.wrapMode}");
                        }
                    }
                }
            }
